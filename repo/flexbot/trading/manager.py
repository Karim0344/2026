...
def _modify_sl(ticket: int, new_sl: float, new_tp: float | None = None) -> bool:
    ...
    if res.retcode != mt5.TRADE_RETCODE_DONE:
        logging.error(f"SLTP failed ticket={ticket} retcode={res.retcode} comment={res.comment}")
        return False
    return True

def _deal_profit_for_comment(symbol: str, comment_contains: str, from_dt: datetime, to_dt: datetime) -> float:
...
def manage_batch(state: BatchState,
    ...
    if (not state.be_applied) and (not tp1_open):
        ...
        if prof > 0:
            ...
            be = state.entry_price + (be_buffer_points * point) if state.is_long else state.entry_price - (be_buffer_points * point)
            changed = False
                ...
                if state.is_long:
                    ...
                    if new_sl > (cur_sl + step):
                        if _modify_sl(t, new_sl):
                            logging.info(f"TRAIL_UPDATE ticket={t} sl {cur_sl}->{new_sl}")
                else:
